...
def _(mo):
    ...
    Using the **Rainfall Anomalies** indicator
    [(link)](https://stats.pacificdata.org/vis?lc=en&df[ds]=SPC2&df[id]=DF_CLIMATE_CHANGE&df[ag]=SPC&df[vs]=1.0&av=true&dq=A.RAIN_ANOM.&pd=,&to[TIME_PERIOD]=false&vw=tl)
    from the Pacific Data Hub climate change dataset (`DF_CLIMATE_CHANGE` / `RAIN_ANOM`).

    Rainfall anomalies measure how much annual rainfall differs from a long-term reference
    period. Positive values = wetter than average; negative values = drier than average.
    """)
    return
...
def _(sdmx):
    ...
    key = dict(CLIMATE_CHANGE_INDICATORS="RAIN_ANOM")
    keep_cols = ["GEO_PICT", "TIME_PERIOD", "value"]
    params = dict(startPeriod="1990", endPeriod="2024")
...
def _(df, font, mo, plt, sns):
    ...
    def boxplot(start_year=min_year, end_year=max_year):
        ...
        ax.set_ylabel("Rainfall anomaly", font=font, size=12)
        ...
        fig.suptitle(
            "Boxplot of rainfall anomalies per year",
            font=font,
            size=20,
        )
        return fig
...
def _(df, font, load_cmap, math, max_year, min_year, np, plt):
    def lineplot(start_year=min_year, end_year=max_year):
        ...
        # IQR band: middle 50% of countries each year
        ax.fill_between(
            years,
            lower,
            upper,
        ...
        fig.suptitle(
            f"Rainfall anomalies from {start_year} to {end_year}",
            font=font,
            size=20,
        )
...
def _(ax_text, df, fig_text, font, load_cmap, max_year, min_year, np, plt):
    def lineplot_labeled(x_min=None):
        ...
        fig_text(
            fig=fig,
            s="Rainfall anomalies across Pacific island countries",
            font=font,
            size=18,
            y=0.98,
            x=0.1,
        ...
        fig_text(
            fig=fig,
            s="Country series with <regional mean> highlighted. Zero = long-term average rainfall.",
            font=font,
            size=12,
            y=0.94,
            x=0.1,
...
def _(mo):
    ...
    Explore whether rainfall anomalies in the latest year show a spatial pattern across
    Pacific EEZs.
    """)
    return
...
def _(ccrs, df, gpd):
    ...
    eez_rain["centroid"] = centroids.apply(
        lambda p: type(p)(wrap_lon(p.x), p.y)
    )
    return eez_rain, latest_year, projection


@app.cell
def _(eez_rain, load_cmap, mpl):
    max_val = eez_rain["value"].max()
    min_val = eez_rain["value"].min()
    abs_max = max(abs(min_val), abs(max_val))

    # Diverging around 0 (wetter vs drier)
    colormap = load_cmap("Earth", cmap_type="continuous")
    norm = mpl.colors.TwoSlopeNorm(vmin=-abs_max, vcenter=0, vmax=abs_max)
    sm = mpl.cm.ScalarMappable(cmap=colormap, norm=norm)

    eez_rain["color"] = eez_rain["value"].apply(
...
@app.cell
def _(abs_max, ax_text, ccrs, eez_rain, latest_year, plt, projection, sm):
    def plot_pacific_rainfall():
        fig, ax = plt.subplots(
            figsize=(10, 8),
        ...
        eez_rain.plot(
            ...
            color=eez_rain["color"],
            edgecolor="white",
            linewidth=0.2,
        )

        for _, row in eez_rain.iterrows():
        ...
        cbar.set_label(f"Rainfall anomaly ({latest_year})")
        cbar.ax.set_ylim(-abs_max, abs_max)

        ax.set_title(f"Rainfall anomalies by EEZ — {latest_year}", size=14)
        return fig

    plot_pacific_rainfall()
    return


@app.cell
def _(eez_rain):
    eez_rain[["COUNTRY", "REF_AREA", "TIME_PERIOD", "value", "color"]]
    return
...
def _(mo):
    ...
    - Which years stand out as regionally wetter or drier than the long-term mean?
    - Do countries share the same anomaly sign in El Niño / La Niña years?
    - How do rainfall anomalies compare with sea-surface temperature anomalies
